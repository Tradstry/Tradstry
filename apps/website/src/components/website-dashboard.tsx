...
const APP_URL = process.env.NEXT_PUBLIC_APP_URL ?? "https://tradstry.com";
...
export function WebsiteDashboard() {
  ...
  const fetcher = React.useMemo(
    () => createHttpGraphQLFetcher({ endpoint: GRAPHQL_ENDPOINT, getToken: auth.getToken }),
    [auth.getToken],
  );
  const subscriber = React.useMemo(
    () => createWebSocketGraphQLSubscriber({ endpoint: GRAPHQL_ENDPOINT, getToken: auth.getToken }),
    [auth.getToken],
  );
  ...
  const platform = React.useMemo<TradstryPlatform>(() => ({
    ...
    pathname,
    appBaseUrl: typeof window === "undefined" ? APP_URL : window.location.origin,
