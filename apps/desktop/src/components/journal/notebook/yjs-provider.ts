...
import type { Unlisten } from "../../../ipc/contract";
import { fromBase64, toBase64 } from "@tradstry/notebook-core";
import { appendNoteUpdate, noteUpdates } from "../../../backend";

/** The desktop sync service emits this after storing new update blobs. Payload: note ids. */
 ...
 * touch the network — the auth token lives only in the main process — so updates
 * travel through the Electron preload bridge, and the sync loop ships the outbox rows.
 */
export function createLocalProvider(
  ...
  return {
    ...
    connect() {
      ...
      const off = window.tradstry.listen<string[]>(NOTE_UPDATES_EVENT, (event) => {
