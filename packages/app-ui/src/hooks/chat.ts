...
import * as chatService from "@tradstry/app-ui/lib/service/chat";
import type {
	ChatContext,
	ChatMessage,
	ChatSession,
...
import { preserveMessageContexts } from "@tradstry/app-ui/lib/types/chat";
...
export function useSendMessage(workspaceId: string | null) {
	...
	const queryClient = useQueryClient();
	const store = useChatStore();
	const activeSubscription = useRef<(() => void) | null>(null);

	const stopSubscription = () => {
		const unsubscribe = activeSubscription.current;
	...
	>({
		...
		onSuccess: (jobId, { sessionId, context }) => {
			...
			activeSubscription.current = subscriber<{ chatStream: ChatStreamEvent }>(
