...
function ConnectionCard({ account }: { account: Account }) {
  ...
  // Derived, not a separate API: the backend skips syncing the moment this
  // meter is full, so the meter is the state.
  const { data: billing } = useBilling();
  const dataMeter = billing?.meters.data;
  const dataCapReached =
    dataMeter?.limit != null && dataMeter.used >= dataMeter.limit;
    ...
    <div className="rounded-lg border p-3">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-1">
          {dataCapReached && (
            <span
              className="rounded bg-amber-500/10 px-1.5 py-0.5 text-[0.6rem] font-medium text-amber-600"
              title="Automatic sync is paused because your plan's data limit is full. Your existing trades are untouched — upgrade or free up space to resume."
            >
              Sync paused
            </span>
          )}
