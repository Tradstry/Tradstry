...
impl PostgresSaver {
    pub fn new(connection_string: impl Into<String>) -> Result<Self, CheckpointError> {
        ...
        let mut client = saver.open_client()?;
    ...
    fn open_client(&self) -> Result<Client, CheckpointError> {
    ...
    fn initialize_schema(client: &mut Client) -> Result<(), CheckpointError> {
        client
            .batch_execute(
                r#"
            ...
            .map_err(|err| {
                CheckpointError::storage(format!("failed to initialize postgres schema: {err}"))
            })
    }
    ...
    fn load_pending_writes(
        ...
        let rows = client
            ...
            .map_err(|err| {
                CheckpointError::storage(format!("failed to query pending writes: {err}"))
            })?;
        ...
        let mut pending_writes = Vec::with_capacity(rows.len());
        for row in rows {
            let task_id: String = row.get(0);
            let channel: String = row.get(1);
            let value_json: String = row.get(2);
            let task_path: String = row.get(3);

            let value = serde_json::from_str(&value_json).map_err(|err| {
                CheckpointError::serialization(format!(
                    "failed to deserialize pending write value for task '{task_id}': {err}"
                ))
            })?;
...
impl CheckpointSaver for PostgresSaver {
    fn get_tuple(
        ...
        let row = if let Some(checkpoint_id) = &config.checkpoint_id {
            client
                .query_opt(
                    ...
                    WHERE thread_id = $1
                      ...
                      AND checkpoint_id = $3
                    "#,
                    &[&config.thread_id, &config.checkpoint_ns, checkpoint_id],
                )
                .map_err(|err| {
            ...
            client
                ...
                .map_err(|err| {
                    CheckpointError::storage(format!(
                        "failed to fetch latest checkpoint tuple: {err}"
                    ))
                })?
        };
        ...
        let metadata: CheckpointMetadata = serde_json::from_str(&metadata_json).map_err(|err| {
            CheckpointError::serialization(format!(
                "failed to deserialize metadata '{checkpoint_id}': {err}"
            ))
        })?;
    ...
    fn list(&self, query: &ListCheckpointsQuery) -> Result<Vec<CheckpointTuple>, CheckpointError> {
        ...
        for row in rows {
            ...
            let checkpoint_ns: String = row.get(1);
            ...
            let parent_checkpoint_id: Option<String> = row.get(5);
            ...
            let pending_writes =
                Self::load_pending_writes(&mut client, &thread_id, &checkpoint_ns, &checkpoint_id)?;
    ...
    fn put(
        ...
        let metadata_json = serde_json::to_string(&metadata).map_err(|err| {
            CheckpointError::serialization(format!(
                "failed to serialize metadata '{checkpoint_id}': {err}"
            ))
        })?;
        ...
        .map_err(|err| {
            CheckpointError::storage(format!(
                "failed to persist checkpoint '{checkpoint_id}': {err}"
            ))
        })?;
    ...
    fn put_writes(
        ...
        for (idx, (channel_name, value)) in writes.iter().enumerate() {
            ...
            if write_idx >= 0 {
                ...
                .map_err(|err| {
                    CheckpointError::storage(format!(
                        "failed inserting regular write for task '{task_id}': {err}"
                    ))
                })?;
            } else {
                tx.execute(
                ...
                .map_err(|err| {
                    CheckpointError::storage(format!(
                        "failed upserting special write for task '{task_id}': {err}"
                    ))
                })?;
            }
        }
    ...
    fn delete_thread(&self, thread_id: &str) -> Result<(), CheckpointError> {
        ...
        tx.execute("DELETE FROM writes WHERE thread_id = $1", &[&thread_id])
            .map_err(|err| {
                CheckpointError::storage(format!(
                    "failed deleting writes for thread '{thread_id}': {err}"
                ))
            })?;
        tx.execute(
        ...
        .map_err(|err| {
            CheckpointError::storage(format!(
                "failed deleting checkpoints for thread '{thread_id}': {err}"
            ))
        })?;
    ...
    fn copy_thread(
        ...
        let source_exists = tx
            ...
            .map_err(|err| {
                CheckpointError::storage(format!(
                    "failed to check source thread '{source_thread_id}': {err}"
                ))
            })?
            .is_some();
            ...
            PruneStrategy::KeepLatest => {
                for thread_id in thread_ids {
                    let latest_rows = tx
                        ...
                        .map_err(|err| {
                            CheckpointError::storage(format!(
                                "failed querying latest checkpoints for thread '{thread_id}': {err}"
                            ))
                        })?;

                    let latest_by_namespace = latest_rows
                        .into_iter()
                    ...
                    let all_rows = tx
                        ...
                        .map_err(|err| {
                            CheckpointError::storage(format!(
                                "failed querying prune candidates for thread '{thread_id}': {err}"
                            ))
                        })?;

                    let to_delete = all_rows
                        .into_iter()
                        .filter_map(|row| {
                            let checkpoint_ns: String = row.get(0);
                            ...
                            if keep {
                                ...
                                Some((checkpoint_ns, checkpoint_id))
                            }
                        })
                        .collect::<Vec<_>>();
                    ...
                    for (checkpoint_ns, checkpoint_id) in to_delete {
                        ...
                        .map_err(|err| {
                            CheckpointError::storage(format!(
                                "failed deleting pruned checkpoint '{checkpoint_id}' for thread '{thread_id}': {err}"
                            ))
                        })?;
                    }
                }
...
mod tests {
    ...
    fn with_saver(test_name: &str, run: impl FnOnce(&PostgresSaver, &str)) {
        ...
        let scope = format!("lg_rs_pg_{test_name}_{}", Uuid::new_v4().simple());
        run(&saver, &scope);
    }
    ...
    fn put_and_get_tuple_returns_latest_checkpoint() {
        with_saver("put_get_latest", |saver, scope| {
            let thread = format!("{scope}-thread");
            let base = CheckpointConfig::new(thread.clone());
            let first = saver
                .put(
                    &base,
                    checkpoint("0001", 1),
            ...
            saver.delete_thread(&thread).unwrap();
        });
    }

    #[test]
    fn put_writes_dedupes_regular_indices_and_overwrites_special_channels() {
        with_saver("put_writes", |saver, scope| {
            let thread = format!("{scope}-thread");
            let config = saver
                .put(
                    &CheckpointConfig::new(thread.clone()),
                    checkpoint("0001", 1),
                    CheckpointMetadata::default(),
                    BTreeMap::new(),
                )
    ...
    fn list_applies_before_limit_and_metadata_filter() {
        with_saver("list_filters", |saver, scope| {
            let thread = format!("{scope}-thread");
            ...
            let listed = saver
                .list(&ListCheckpointsQuery {
                    config: Some(CheckpointConfig::new(thread.clone())),
                    metadata_filter: BTreeMap::from([("run_id".to_owned(), json!("run-b"))]),
                    before: Some(CheckpointConfig::new(thread.clone()).with_checkpoint_id("0003")),
                    limit: Some(5),
                })
                .unwrap();
    ...
    fn delete_thread_removes_checkpoints_and_writes() {
        with_saver("delete_thread", |saver, scope| {
            let thread = format!("{scope}-thread");
            let config = saver
                .put(
                    &CheckpointConfig::new(thread.clone()),
            ...
            assert!(
                saver
                    .get_tuple(&CheckpointConfig::new(thread))
                    .unwrap()
                    .is_none()
            );
        });
    ...
    fn copy_thread_clones_checkpoints_and_writes() {
        with_saver("copy_thread", |saver, scope| {
            let source_thread = format!("{scope}-source");
            let target_thread = format!("{scope}-target");
    ...
    fn prune_keep_latest_and_delete_for_runs_work() {
        with_saver("prune_delete_runs", |saver, scope| {
            let thread = format!("{scope}-thread");
