...
pub fn build(deps: Arc<ComparisonDeps>) -> Result<CompiledStateGraph, GraphError> {
    ...
    graph.add_node("resolve_trades", move |state: Value, _ctx: ExecutionContext<'_>| {
    ...
    graph.add_node("fetch_details", move |state: Value, _ctx: ExecutionContext<'_>| {
        ...
        rt.block_on(async move {
            ...
            let trade_a = tools::db_query::execute(
                &args_a,
                &deps.user_id,
                &deps.account_id,
                &deps.turso,
            ...
            let trade_b = tools::db_query::execute(
                &args_b,
                &deps.user_id,
                &deps.account_id,
                &deps.turso,
