...
/// Estimate token count with the chars/4 heuristic (no tokenizer dependency).
...
/// preferring sentence boundaries, falling back to words.
fn split_oversized(text: &str) -> Vec<String> {
    ...
    for unit in units {
        ...
        if estimate_tokens(&candidate) > CHUNK_MAX_TOKENS && !current.is_empty() {
...
pub fn chunk_blocks(blocks: &[Block]) -> Vec<Chunk> {
    ...
    let total: usize = blocks.iter().map(|b| estimate_tokens(&b.text)).sum();
    ...
    for (i, block) in blocks.iter().enumerate() {
        let bt = estimate_tokens(&block.text);
        ...
        if buf_tokens + bt > CHUNK_TARGET_TOKENS && !buf.is_empty() {
            ...
            if let Some(prev) = carry
                && (estimate_tokens(&prev) as f32)
                    <= CHUNK_TARGET_TOKENS as f32 * (CHUNK_OVERLAP_PCT + 0.5)
            {
                buf_tokens += estimate_tokens(&prev);
                buf.push(prev);
            }
        }
