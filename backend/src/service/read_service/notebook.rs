...
use crate::service::db::schema::tables::notebook_images::{self, NotebookImage};
...
pub async fn find_notebook_image(
    ...
    notebook_images::find_notebook_image(user_db.pool(), media_id, user_db.user_id()).await
}
...
pub async fn list_notebook_notes(
    ...
    notebook_table::list_notebook_notes(user_db.pool(), user_db.user_id(), account_id).await
}

pub async fn get_notebook_note(user_db: &UserDb, id: &str) -> Result<Option<NotebookNote>> {
    notebook_table::find_notebook_note(user_db.pool(), id, user_db.user_id()).await
}
...
pub async fn create_notebook_note(
    ...
    notebook_table::create_notebook_note(user_db.pool(), user_db.user_id(), input).await
}
...
pub async fn update_notebook_note(
    ...
    notebook_table::update_notebook_note(user_db.pool(), id, user_db.user_id(), input).await
}

pub async fn delete_notebook_note(user_db: &UserDb, id: &str) -> Result<bool> {
    notebook_table::delete_notebook_note(user_db.pool(), id, user_db.user_id()).await
}
...
pub async fn list_notebook_folders(
    ...
    notebook_folders_table::list_notebook_folders(user_db.pool(), account_id).await
}
...
pub async fn create_notebook_folder(
    ...
    notebook_folders_table::create_notebook_folder(user_db.pool(), input).await
}

pub async fn rename_notebook_folder(user_db: &UserDb, id: &str, name: &str) -> Result<()> {
    notebook_folders_table::rename_notebook_folder(user_db.pool(), id, name).await
}

pub async fn move_notebook_node(user_db: &UserDb, input: MoveNotebookNodeInput) -> Result<()> {
    notebook_folders_table::move_notebook_node(user_db.pool(), input).await
}
...
pub async fn delete_notebook_folder(user_db: &UserDb, folder_id: &str) -> Result<Vec<String>> {
    ...
    let public_ids =
        notebook_folders_table::gather_subtree_image_public_ids(user_db.pool(), folder_id).await?;

    // 2. Remove the DB rows for the whole subtree.
    notebook_folders_table::delete_notebook_folder_subtree(user_db.pool(), folder_id).await?;
